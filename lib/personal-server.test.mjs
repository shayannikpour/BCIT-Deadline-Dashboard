...
test('accounts accept any non-empty password without length requirements', async () => {
  const { call } = fixture();
  const response = await call({
    action: 'signup',
  ...
  const state = await (await call(undefined, cookie)).json();
  assert.equal(state.user.username, 'short_password');
});
